...
class JdbcConnect {
  ...
  def insert(stageInfo: StageInfo): JdbcConnect = {
    val sql = s"insert into metrics(function_name, stage_id, details, num_tasks, name, " +
      s"submission_time, completion_time, executor_run_time, result_size, jvm_gc_time, peak_execution_memory, disk_bytes_spilled) " +
      ...
      s"${stageInfo.taskMetrics.jvmGCTime}, ${stageInfo.taskMetrics.peakExecutionMemory}, ${stageInfo.taskMetrics.diskBytesSpilled})"
