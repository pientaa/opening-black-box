...
class MaxWholeSaleCostTest
  ...
  test("max_cs_wholesale_cost null test") {
    ...
    val sourceDF = UDAF
      .max_cs_wholesale_cost(CatalogSalesStub.threeCatalogSales.toDS())
      .sort("cs_sold_date_sk", "cs_quantity")
      .toDF()
    import udf.model.CS_WholeSaleMaxGroupedBySoldDateAndQuantity
    val expectedDF = Seq(
      CS_WholeSaleMaxGroupedBySoldDateAndQuantity(null, null, Option(BigDecimal.valueOf(10.0))),
      CS_WholeSaleMaxGroupedBySoldDateAndQuantity(null, Option(200), Option(BigDecimal.valueOf(10.0))),
      CS_WholeSaleMaxGroupedBySoldDateAndQuantity(Option(1), null, Option(BigDecimal.valueOf(10.0)))
    ).toDF()
